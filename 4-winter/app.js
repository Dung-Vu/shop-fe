/* ==========================================================================
   AURA Dashboard Engine - Winter Style (HIGH-END FRONTEND V4.0 - NO AUDIO)
   ========================================================================== */
...
(function () {
  ...
  const storageKey = 'aura_data_winter';
  window.AuraConfig = {
    particleCount: 75,
    particleSpeed: 1.0,
    springK: 0.08,
  ...
  function renderChart() {
    ...
    if (isForecastMode && revForecast.predictions.length > 0) {
      ...
      revForecastLine.setAttribute('stroke', '#ffffff');
      ...
      costForecastLine.setAttribute('stroke', '#d4af37');
      ...
      ciArea.setAttribute('fill', '#ffffff');
      ...
      if (isForecastNode) {
        ...
        tooltipContent = `
          ...
          <div class="tooltip-row">
            <span>Doanh thu:</span>
            <strong style="color: #ffffff">$${revVal.toLocaleString()} <span style="font-size: 9px; font-weight: normal; color: var(--color-text-secondary);">[CI: $${revLow}-$${revHigh}]</span></strong>
          </div>
          <div class="tooltip-row">
            <span>Chi phí:</span>
            <strong style="color: #d4af37">$${costVal.toLocaleString()} <span style="font-size: 9px; font-weight: normal; color: var(--color-text-secondary);">[CI: $${costLow}-$${costHigh}]</span></strong>
          </div>
        `;
      } else {
        tooltipContent = `
          ...
          <div class="tooltip-row">
            <span>Doanh thu:</span>
            <strong style="color: #ffffff">$${data.revenue[nearestIndex].toLocaleString()}</strong>
          </div>
          <div class="tooltip-row">
            <span>Chi phí:</span>
            <strong style="color: #d4af37">$${data.cost[nearestIndex].toLocaleString()}</strong>
          </div>
        `;
      }
  ...
  function initVisualPerks() {
    ...
    chartSvg.addEventListener('mousemove', (e) => {
      ...
      gridPaths.forEach(line => {
        ...
        if (Math.abs(distY) < 22 && mouseX > 50 && mouseX < 550) {
          line.isHovered = true;
          // Dampen based on horizontal coordinate offset from center
          const xInfluence = Math.exp(-Math.abs(mouseX - 300) / 160);
          line.displacement = distY * xInfluence * 0.55;
          line.element.setAttribute('stroke', 'rgba(255, 255, 255, 0.35)');
        } else {
    ...
    // F. Canvas Background FBM Blizzard Snowflake Simulation
    ...
    class Snowflake {
      ...
      reset() {
        ...
        this.size = Math.random() * 3 + 1.2; // snowflake size
      ...
      update(mouseX, mouseY, t) {
        // Blizzard Wind from FBM noise field coordinates
        const scale = 0.005;
        const windAngle = fbm(this.x * scale, this.y * scale + t) * Math.PI * 2;
        
        // Blizzard Wind from FBM noise field coordinates (R3)
        const speedMultiplier = window.AuraConfig.particleSpeed;
        const targetVx = (Math.cos(windAngle) * 1.5 + this.speedX) * speedMultiplier;
        const targetVy = (Math.sin(windAngle) * 0.8 + this.speedY) * speedMultiplier;
        ...
        // Mouse interactive repelling vortices <= 140px
    ...
    const flakes = Array.from({ length: 75 }, () => new Snowflake());

    // Handle dynamic counts of particles via settings (R3)
    window.__adjustParticles = (newCount) => {
      while (flakes.length < newCount) {
        flakes.push(new Snowflake());
      }
      if (flakes.length > newCount) {
        flakes.length = newCount;
      }
    };
    ...
    function tick() {
      ...
      flakes.forEach(flake => {
        flake.update(mouseRawX, mouseRawY, t);
        flake.draw();
      });
