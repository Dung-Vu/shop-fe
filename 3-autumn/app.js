/* ==========================================================================
   AURA Dashboard Engine - Autumn Style (HIGH-END FRONTEND V4.0)
   ========================================================================== */
...
(function () {
  ...
  const storageKey = 'aura_data_autumn';
  window.AuraConfig = {
    particleCount: 75,
    particleSpeed: 1.0,
    springK: 0.08,
      ...
      try {
        ...
        if (parsed.config) {
          ...
          if (slideOpacity) {
            ...
            const canvasEl = document.getElementById('ambient-leaves');
            if (canvasEl) {
  ...
  // 3. Render Interactive SVG Chart with Smooth Bézier Curves & C  function renderChart() {
    ...
    // Draw Glowing Dynamic Revenue Flow Line
    ...
    if (isForecastMode && revForecast.predictions.length > 0) {
      ...
      revForecastLine.setAttribute('stroke', '#ff7b00');
      ...
      costForecastLine.setAttribute('stroke', '#7a8258');
      ...
      ciArea.setAttribute('fill', '#ff7b00');
    ...
    crosshair.setAttribute('stroke', 'rgba(255, 107, 59, 0.4)');
      ...
      if (isForecastNode) {
        ...
        tooltipContent = `
          ...
          <div class="tooltip-row">
            <span>Doanh thu:</span>
            <strong style="color: #ff6b3b">$${revVal.toLocaleString()} <span style="font-size: 9px; font-weight: normal; color: var(--color-text-secondary);">[CI: $${revLow}-$${revHigh}]</span></strong>
          </div>
          <div class="tooltip-row">
            <span>Chi phí:</span>
            <strong style="color: #7a8258">$${costVal.toLocaleString()} <span style="font-size: 9px; font-weight: normal; color: var(--color-text-secondary);">[CI: $${costLow}-$${costHigh}]</span></strong>
          </div>
        `;
      } else {
        tooltipContent = `
          ...
          <div class="tooltip-row">
            <span>Doanh thu:</span>
            <strong style="color: #ff6b3b">$${data.revenue[nearestIndex].toLocaleString()}</strong>
          </div>
          <div class="tooltip-row">
            <span>Chi phí:</span>
            <strong style="color: #7a8258">$${data.cost[nearestIndex].toLocaleString()}</strong>
          </div>
        `;
      }
  ...
  /* ==========================================================================
     AURA VISUAL PERKS V4.0 - FBM Noise Leaf Field & Spring Elastic SVG Grid & Parallax
     ========================================================================== */

  function initVisualPerks() {
    const themeColorRgb = '255, 107, 59'; // Autumn Orange tint
    ...
    styleElement.textContent = `
      ...
      .kpi-card::after, .chart-container::after {
        ...
        background: radial-gradient(circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(255, 107, 59, 0.08) 0%, transparent 60%);
        pointer-events: none;
        z-index: 5;
        opacity: 0;
    ...
    // Hook elastic grid plucking near mouse (no audio)
    chartSvg.addEventListener('mousemove', (e) => {
      ...
      gridPaths.forEach(line => {
        ...
        if (Math.abs(distY) < 22 && mouseX > 50 && mouseX < 550) {
          ...
          line.element.setAttribute('stroke', 'rgba(255, 107, 59, 0.3)');
        } else {
    ...
    // F. Canvas Background FBM Wind Leaf Simulation Setup
    ...
    const leafColors = [
      '255, 107, 59', // orange
      '229, 169, 59', // gold
      '196, 108, 59', // dark burnt orange
      '122, 130, 88', // soft olive
      '180, 70, 50'   // crimson red
    ];

    // True 4-octave FBM noise generator
    function fbm(x, y, t) {
      let value = 0;
      let amplitude = 1.0;
      let frequency = 0.003;
    ...
    class Leaf {
      ...
      reset(initial = false) {
        ...
        this.size = Math.random() * 8 + 6; // leaf size
        this.speedY = Math.random() * 0.8 + 0.5; // falling rate
        this.speedX = Math.random() * 0.5 - 0.1; // slide drift
        this.vx = 0;
        this.vy = 0;
        this.baseColor = leafColors[Math.floor(Math.random() * leafColors.length)];
      ...
      update(mX, mY, time) {
        ...
        // 4-octave FBM wind field angles
        const scale = 0.002;
        const windAngle = fbm(this.x * scale, this.y * scale, time) * Math.PI * 2.0;
        
        // Wind velocity with sway oscillations (R3)
        const speedMultiplier = window.AuraConfig.particleSpeed;
        const targetVx = (Math.cos(windAngle) * 0.6 + this.speedX + Math.sin(this.swayValue) * 0.3) * speedMultiplier;
        const targetVy = (Math.sin(windAngle) * 0.4 + this.speedY) * speedMultiplier;
        
        this.vx += (targetVx - this.vx) * 0.05;
        this.vy += (targetVy - this.vy) * 0.05;
        
        // Mouse attract/swirl velocity deflection forces within 150px
        if (mX !== undefined && mY !== undefined) {
          ...
          if (dist < 150) {
            ...
            // Soft inertia-based velocity additions
            this.vx += (swirlX * 0.6 + attractX * 0.4) * force;
            this.vy += (swirlY * 0.4 + attractY * 0.3) * force;
          }
        }
        ...
        this.angle += this.angleSpeed + (this.vx * 0.02);

        if (this.age > this.life || this.y > height + 20 || this.x < -20 || this.x > width + 20) {
          this.reset(false);
        }
      }
      draw() {
        const currentAlpha = this.alpha * Math.min(1, (this.life - this.age) / 80);
        ctx.save();
        ...
        ctx.strokeStyle = `rgba(255, 255, 255, ${currentAlpha * 0.25})`;
        ctx.lineWidth = 1;
        ctx.stroke();
    ...
    const leaves = Array.from({ length: 75 }, () => new Leaf());

    // Handle dynamic counts of particles via settings (R3)
    window.__adjustParticles = (newCount) => {
      while (leaves.length < newCount) {
        leaves.push(new Leaf());
      }
      if (leaves.length > newCount) {
        leaves.length = newCount;
      }
    };
    ...
    function unifiedTick() {
      ...
      // 3. Elastic Spring-Mass decay for SVG Grid Lines (R3)
      const dt = 0.16;
      ...
      // 4. Fluid Leaf particle simulation updates (Floating & swaying in 4-octave FBM wind field)
      ctx.clearRect(0, 0, width, height);
      leafTime += 0.003;
      leaves.forEach(leaf => {
        leaf.update(globalMouseX, globalMouseY, leafTime);
        leaf.draw();
      });
