/* ==========================================================================
   AURA Dashboard Engine - Warm Timber Style (HIGH-END FRONTEND V4.0)
   ========================================================================== */
...
(function () {
  ...
  const storageKey = 'aura_data_timber';
  window.AuraConfig = {
    particleCount: 75,
    particleSpeed: 1.0,
    springK: 0.08,
  ...
  function renderChart() {
    ...
    if (isForecastMode && revForecast.predictions.length > 0) {
      ...
      costForecastLine.setAttribute('stroke', '#4e3b31');
    ...
    crosshair.setAttribute('stroke', 'rgba(229, 169, 59, 0.4)');
      ...
      if (isForecastNode) {
        ...
        tooltipContent = `
          ...
          <div class="tooltip-row">
            <span>Doanh thu:</span>
            <strong style="color: #c08a3e">$${revVal.toLocaleString()} <span style="font-size: 9px; font-weight: normal; color: var(--color-text-secondary);">[CI: $${revLow}-$${revHigh}]</span></strong>
          </div>
          <div class="tooltip-row">
            <span>Chi phí:</span>
            <strong style="color: #4e3b31">$${costVal.toLocaleString()} <span style="font-size: 9px; font-weight: normal; color: var(--color-text-secondary);">[CI: $${costLow}-$${costHigh}]</span></strong>
          </div>
        `;
      } else {
        tooltipContent = `
          ...
          <div class="tooltip-row">
            <span>Doanh thu:</span>
            <strong style="color: #c08a3e">$${data.revenue[nearestIndex].toLocaleString()}</strong>
          </div>
          <div class="tooltip-row">
            <span>Chi phí:</span>
            <strong style="color: #4e3b31">$${data.cost[nearestIndex].toLocaleString()}</strong>
          </div>
        `;
      }
  ...
  function initVisualPerks() {
    const themeColorRgb = '229, 169, 59'; // Amber/Timber gold tint
    ...
    styleElement.textContent = `
      ...
      .kpi-card::after, .chart-container::after {
        ...
        background: radial-gradient(circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(229, 169, 59, 0.08) 0%, transparent 60%);
        pointer-events: none;
        z-index: 5;
        opacity: 0;
    ...
    chartSvg.addEventListener('mousemove', (e) => {
      ...
      gridPaths.forEach(line => {
        ...
        if (Math.abs(distY) < 22 && mouseX > 50 && mouseX < 550) {
          ...
          line.element.setAttribute('stroke', 'rgba(229, 169, 59, 0.3)');
        } else {
    ...
    class FluidParticle {
      ...
      reset(initial = false) {
        ...
        this.riseSpeed = Math.random() * 0.9 + 0.4; // Ember rising velocity bias
        this.vx = 0;
        this.vy = 0;
        ...
        const colors = [
          '229, 169, 59', // Amber
          '192, 138, 62', // Wood Gold
          '244, 91, 50',  // Warm Orange-Red
          '255, 172, 51'  // Bright Gold
        ];
      ...
      update(mX, mY, time) {
        ...
        // Vector Field: organic noise vector grid (4-octave FBM representation)
        ...
        // Rising sparks move upwards (-y)
        const targetVy = Math.sin(angle) * this.speed * speedMultiplier - this.riseSpeed * speedMultiplier;
        
        this.vx += (targetVx - this.vx) * 0.08;
        this.vy += (targetVy - this.vy) * 0.08;
        
        // Mouse interactive swirl / vortex forces within 180px
        if (mX !== undefined && mY !== undefined) {
          ...
          if (dist < 180) {
            ...
            this.vx += swirlX * force * 1.5;
            this.vy += swirlY * force * 1.5;
          }
        }
    ...
    function unifiedTick() {
      ...
      // 4. Fluid Particle simulation updates (Sparks rising & swirling in FBM noise)
